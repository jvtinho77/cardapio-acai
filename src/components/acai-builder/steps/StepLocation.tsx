...
export function StepLocation({ onSelect, currentLocation }: StepLocationProps) {
    ...
    const initialPos = currentLocation || defaultCenter;
    ...
    useEffect(() => {
        ...
        // If we already have a location, show marker
        if (currentLocation) {
    ...
    useEffect(() => {
        if (!currentLocation && !markerInstance.current) {
            handleGetLocation();
        }
    }, []);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="relative h-[300px] w-full rounded-xl overflow-hidden border border-gray-200 shadow-md">
                ...
                {!currentLocation && !loading && !markerInstance.current && (
